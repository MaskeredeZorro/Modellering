...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    model.x=pyomo.Var(model.periods, within=pyomo.NonNegativeIntegers, bounds=(0,data["max_prod"])) #vi tilføjer bounds for NonNegativeIntegers
    model.y=pyomo.Var(model.periods, within=pyomo.Binary)
    model.s=pyomo.Var(model.periods_and_zero,within=pyomo.NonNegativeIntegers, bounds=(0,data["max_inv"])) #vi er 0 baseret i python. Den 1. periode starter i 0. Så for at fange 0 har vi lavet model.periods_and_zero
    # Add objective function
    model.obj=pyomo.Objective(expr=sum(model.p[t]*model.x[t]+model.q[t]*model.y[t]+model.h[t]*model.s[t] for t in model.periods))

    # Add flow conservation constraints
    model.flowConstraints=pyomo.ConstraintList()
    for t in model.periods:
        model.flowConstraints.add(expr=model.s[t-1]+model.x[t]==model.s[t]+model.demands[t])
