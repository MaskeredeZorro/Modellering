...
#-----------------------------------------------#
#Location allocation med multi sourcing.
#Vi ønsker her at minimere den aggregerede afstand fra kommunerne til deres respektive sygehus.
#   Forskellen her er dog, at vi har med multisourcing at gøre. Det er basically opg. 2 og 3. men med multi sourcing!
#Når vi har med multisourcing at gøre, skal vi blot ændre vores model.x variabel til at være NonNegativeReals.
#   Så kan den antage værdier x\in{0;1} og dermed enten allokere en kommune kontinuert.
#-----------------------------------------------#
...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    model.x = pyomo.Var(model.kommune, model.kommune, within=pyomo.NonNegativeReals)
...
###-----------------------Vi laver en location-allocation (CFLP model-------------------------###
    ...
    # Add max budget
    model.maxcost=pyomo.Constraint(expr=sum(model.etablering[i] * model.y[i] for i in model.kommune)<=model.maxbudget)

    # Add extra udbud til sygehusets basis kapacitet
...
def displaySolution(model: pyomo.ConcreteModel()):
    #Print optimal objective funktion værdi
    print('Optimal objective function value is', pyomo.value(model.obj))
    #Print de åbne faciliteter
    for i in model.kommune:
        if pyomo.value(model.y[i])==1:
            ...
            for j in model.kommune:
                if pyomo.value(model.x[i,j]) >0:
                    print(model.kommune_labels[j],"(",pyomo.value(model.x[i,j])*100, "%)",end=',')
            print('\n')
