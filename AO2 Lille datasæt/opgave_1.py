...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    model.kommune_labels = data['inhab2022']
    model.kommune_labels30 = data['inhab2030']
    ...
    model.obj=pyomo.Objective(expr=sum(model.kommune_labels30[j]*model.travel[i][j]*model.x[i,j] for i in model.kommune for j in model.kommune))

    #tidligere objektfunktion:
    #model.obj=pyomo.Objective(expr=sum(model.etablering[i] * model.y[i] + model.extra[i] * model.z[i] for i in model.kommune))


    #måske lav et forloop for i og så en sum for j. Tilføj model.travel[i][j]
    #model.obj=pyomo.ObjectiveList(
    #    for j in model.kommune:
    #        expr=sum(model.travel[i]*model.y[i] for i in model.kommune),sense = pyomo.minimize)
    #model.obj = pyomo.Objective(expr=sum(model.etablering[i] * model.y[i] + model.extra[i] * model.z[i] for i in model.kommune), sense=pyomo.minimize)
    ...
    for i in model.kommune:
        model.capacities.add(expr=sum(model.kommune_labels[j] * model.x[i,j] for j in model.kommune) <= model.basis_capacity[i] * model.y[i]+1000*model.z[i]) #+1000*model.z_1

    # Add the capacity constraints
    model.capacities2 = pyomo.ConstraintList()
    for i in model.kommune:
        model.capacities2.add(expr=sum(model.kommune_labels30[j] * model.x[i,j] for j in model.kommune) <= model.basis_capacity[i] * model.y[i]+1000*model.z[i]) #+1000*model.z_1
